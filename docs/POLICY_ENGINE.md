...
Fichiers : `bin/policy.py` · `config/policy.yaml` ·
...
`config/policy.yaml`. Aucun code à toucher — contrat du lot, point 2. Les
...
`config/policy.yaml` liste les directions gouvernées. Une direction absente
...
Growth ni le Marketing n'ont de canal déclaré dans `policy.yaml`. Le dépôt
...
| `config/policy.yaml` | créé — le lot le disait « à étendre », il n'existait pas | — |
