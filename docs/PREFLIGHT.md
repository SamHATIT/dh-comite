...
> jamais à lui-même.
>
> `bin/preflight.py` · `config/preflight.yaml` · LOT-05 · 17 août 2026
...
direction. Ces outils portent `verifier_aide: false` dans `preflight.yaml`, chacun
...
## 4. Configuration — `config/preflight.yaml`
...
sous-ensemble YAML strictement employé par `preflight.yaml` : mappings, listes de
