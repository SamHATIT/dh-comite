            ...
            for outil in definition.get("outils", []):
                ...
                if nom_analyseur:
                    ...
                    if analyseur is None:
                        raise ValueError(
                            "analyseur inconnu dans policy.yaml : %s" % nom_analyseur
                        )
...
def principal():
    ...
    analyseur.add_argument(
        "--capacites",
        default=os.path.join(racine_depot(), "config", "policy.yaml"),
        help="correspondance outil → capacité (défaut : config/policy.yaml)",
    )
    analyseur.add_argument(
        "--curseurs",
