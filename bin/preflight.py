...
jamais à lui-même : voir la règle du paradoxe, ci-dessous et dans docs/PREFLIGHT.md.
    ...
    preflight.py --lister              ce que déclare config/capabilites.yaml
...
# capabilites.yaml. `--autotest` compare les deux lectures quand PyYAML est présent :
...
def controle_permissions(direction, spec, axes):
    ...
    if not CLE_VALIDE.match(cle):
        return [echec(direction, "permissions", f"clé de curseur invalide : {cle!r}",
                      "corriger config/capabilites.yaml")], []
    ...
    for api in spec.get("apis") or []:
        ...
        if api.get("optionnel"):
            ...
            echecs.append(echec(direction, "apis", motif,
                                f"rétablir {nom} ou le déclarer optionnel dans capabilites.yaml"))
    ...
    for ligne in lignes:
        ...
        if not reconnus:
            avertissements.append(
                ...
                f"ajouter un motif dans config/capabilites.yaml si c'en est un")
            continue
...
def controle_evidence(direction, spec):
    ...
    if not moyens:
        return [echec(direction, "evidence", "aucun moyen de preuve déclaré",
                      "déclarer un dépôt ou une table dans config/capabilites.yaml")], []
    motifs, ok = [], False
    for m in moyens:
        t = m.get("type")
        if t == "table":
...
def autotest(conf):
    ...
    try:
        ...
        if interne != yaml.safe_load(texte):
            print("✗ le parseur de repli diverge de PyYAML sur capabilites.yaml")
            ok = False
        else:
            print("✓ parseur de repli identique à PyYAML sur capabilites.yaml")
    except ImportError:
